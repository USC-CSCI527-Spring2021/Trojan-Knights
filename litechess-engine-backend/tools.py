...
ae_state_dict = torch.load(
    'checkpoints/best_autoencoder.pth.tar', map_location=lambda storage, loc: storage)
siamese_state_dict = torch.load(
    'checkpoints/best_siamese.pth.tar', map_location=lambda storage, loc: storage)
...
def get_bitboard(board):
    ...
    board : chess.pgn board object / chess.fen board object
    ...
    for i in range(64):
        if board.piece_at(i):
            ...
            bitboard[(piece_idx[board.piece_at(
                i).symbol().lower()] + i * 6) * color] = 1
...
def compare(x1, x2):
    ...
    cmp = comparator(x1, x2).detach()
    vals = F.softmax(cmp, dim=1)
    return vals.numpy()
...
def parseFEN(fen):
    ...
    initial_chessboard = chess.Board(FEN_INITIAL)
    ...
    bitboard = get_bitboard(chessboard)
    initial_bitboard = get_bitboard(initial_chessboard)

    bitboard = np.array(bitboard)
    initial_bitboard = np.array(initial_bitboard)

    evaluation = compare(initial_bitboard, bitboard)
    evaluation = (evaluation[0][1] - 0.5) * 10
