...
parser = argparse.ArgumentParser(description='Siamese Regnet')
...
model = Siamese(load_weights=False).to(device)
state = torch.load('/home/parthsuresh/DeepChess/checkpoints/reg_net/lr_10_decay_99/regnet_1.pth.tar', map_location=lambda storage, loc: storage)

model_dict = model.state_dict()
print(model_dict.keys())
print(state['state_dict'].keys())
pretrained_dict = {k: v for k, v in state['state_dict'].items() if k in model_dict and 'encoder' in k}
...
def save(epoch):
    state = {'state_dict': model.state_dict(),
             ...
             'epoch': epoch + 1}
    save_dir = 'checkpoints/siamese_regnet/lr_{}_decay_{}'.format(int(lr*1000), int(decay*100))
    if not os.path.isdir(save_dir):
        os.mkdir(save_dir)
    torch.save(state, os.path.join(save_dir, 'sm_reg_{}.pth.tar'.format(epoch)))
